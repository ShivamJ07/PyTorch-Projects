...
def get_dataset(batch_size: int):
    train_set = datasets.MNIST(
        root='./dataset', train=True, download=True, transform=transforms.Compose([
            ...
            transforms.ToTensor(),
            transforms.Normalize(mean=[0.1308], std=[0.2760])
        ]))
...
def check_accuracy(loader, model):
    ...
    with torch.no_grad():
        ...
        print(f'Correctly classified {(total_correct)}/{total_samples} with an accuracy of {(float(total_correct)/total_samples)*100:.2f}')